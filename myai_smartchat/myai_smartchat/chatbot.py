...
import random
import json
from ddgs import DDGS  # <- updated, no duckduckgo_search anymore


class WebSmartChatbot:
    """Smart chatbot: math solver + keyword match + neural network + DuckDuckGo fallback."""

    def __init__(self, lr=0.01, hidden_size=16):
        self.patterns = {}        # training Q&A
        self.word_index = {}      # vocab -> index
        self.index_word = {}      # index -> vocab
        self.input_size = 0
    ...
    def predict(self, user_input):
        ...
        # 3) Math evaluation
        try:
            expr = user_input.replace('x', '*').replace('X', '*')
        ...
        return self.duckduckgo_search(user_input)
        ...
        try:
            with DDGS() as ddgs:
                ...
                for i, r in enumerate(results_gen):
                    if i >= max_results:
                        break
                    snippet = r.get('body', r.get('title', 'No snippet'))
